...
public class Legislature {
    ...
    private String name;
    /** The title or term for a member of this house, I.E. Senator, Representative, Member. */
    ...
    public Legislature(String name, String title, GovernmentLevel governmentLevel, boolean isUpperHouse, int termLength, int nextElectionYear, int totalSeats) {
        this.name = name;
        this.title = title;
    ...
    public void reassignPartySeats(Party fromParty, Party toParty, int seats) {
        if (seats > totalSeats) throw new IllegalArgumentException("Attempted to set more seats than are available.");
        if (seats > getPartySeats(fromParty)) throw new IllegalArgumentException("Attempted to reassign more seats than were held by the specified 'from' party.");
    ...
    /** Return the members list of this House. Between 0 and {@code totalSeats} characters are generated for any house. */
    ...
    public void addMember(PoliticalActor member) {
        switch (governmentLevel) {
            case LOCAL :
                ...
                else throw new IllegalArgumentException("Members of a local legislature must be local officials.");
                break;
            case STATE :
                if (member instanceof StateOfficial)
                    ((StateOfficial) member).addRole(isUpperHouse ? StateRole.STATE_SENATOR : StateRole.STATE_REPRESENTATIVE);
                else throw new IllegalArgumentException("Members of a state legislature must be state officials.");
                break;
            case FEDERAL :
                ...
                else throw new IllegalArgumentException("Members of a federal legislature must be federal officials.");
                break;
        }
