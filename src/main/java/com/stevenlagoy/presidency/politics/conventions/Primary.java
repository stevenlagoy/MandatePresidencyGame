...
import com.stevenlagoy.presidency.characters.Candidate;
...
public class Primary extends Convention {
    ...
    /**
     * Percentage of the vote which a candidate must receive in order to bind ANY
     * delegates.
     */
    private float qualificationThreshold;
    /**
     * Percentage of the vote which a candidate must receive in order to bind ALL
     * delegates.
     */
    private float takeAllTriggerThreshold;

    public Primary(State state, Party associatedParty, Franchise type, LocalDate date, Collection<Candidate> candidates,
            int numberDelegates) {
        this.state = state;
