...
import com.stevenlagoy.presidency.graphics.entity.terrain.BlendMapTerrain;
import com.stevenlagoy.presidency.graphics.entity.terrain.Terrain;
import com.stevenlagoy.presidency.graphics.entity.terrain.TerrainTexture;
...
public class TestGame implements ILogic {
    ...
    public void init() throws Exception {
        ...
        TerrainTexture backgroundTexture = new TerrainTexture(
                ObjectLoader.loadTexture("src\\main\\resources\\gfx\\textures\\grass.png"));
        TerrainTexture redTerrainTexture = new TerrainTexture(
                ObjectLoader.loadTexture("src\\main\\resources\\gfx\\textures\\red.png"));
        TerrainTexture greenTerrainTexture = new TerrainTexture(
                ObjectLoader.loadTexture("src\\main\\resources\\gfx\\textures\\green.png"));
        TerrainTexture blueTerrainTexture = new TerrainTexture(
                ObjectLoader.loadTexture("src\\main\\resources\\gfx\\textures\\blue.png"));
        TerrainTexture blendMap = new TerrainTexture(
                ObjectLoader.loadTexture("src\\main\\resources\\gfx\\textures\\blendMap.png"));

        BlendMapTerrain blendMapTerrain = new BlendMapTerrain(backgroundTexture, redTerrainTexture, greenTerrainTexture,
                blueTerrainTexture);
        for (int i = -5; i < 5; i++) {
            for (int j = -5; j < 5; j++) {
                sceneManager.addTerrain(
                        new Terrain(new Vector3f(800 * i, 0, 800 * j), terrainMaterial, blendMapTerrain, blendMap));
            }
        }
        ...
        // quadModel.setTexture(orangeTexture);
        // Entity quadEntity = new Entity(quadModel, new Vector3f(0, 0, -1), new
        // Vector3f(0), 1.0f);
        // sceneManager.addEntity(quadEntity);
